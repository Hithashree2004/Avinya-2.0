...
const AnalysisResults: React.FC = () => {
  ...
  // Identify suspicious patches: N1-N2 difference between 1 and 30, limited to top 30 patches
  const suspiciousPatches = patchDataWithNumbers.filter(patch => {
    if (!patch.actualCounts) return false;
    const diff = Math.abs(patch.actualCounts.n1 - patch.actualCounts.n2);
    // Focus on differences between 1 and 30
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="current" className="w-full">
        <TabsList className="grid w-full grid-cols-7">
          ...
          <TabsTrigger value="suspicious">Suspicious Cell Working</TabsTrigger>
        </TabsList>
        ...
        {/* Suspicious Cell Working Tab */}
        <TabsContent value="suspicious" className="space-y-4">
          <Card className="p-6">
            <h3 className="text-lg font-medium mb-4 text-center">Suspicious Cell Working</h3>
            <p className="text-sm text-gray-600 mb-6 text-center">
              A curated list of suspicious patches (1-30 difference in N1/N2 counts) identified for manual verification by medical professionals.
            </p>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Necrosis</TableHead>
                    <TableHead>Tumor Cells</TableHead>
                    <TableHead>N1-N2 Diff</TableHead>
                    <TableHead>Total</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {suspiciousPatches.length > 0 ? (
                    suspiciousPatches.map((patch) => (
                      <TableRow key={patch.patchId}>
                        <TableCell className="font-medium">Patch {patch.displayPatchId}</TableCell>
                        <TableCell>{patch.fileName}</TableCell>
                        <TableCell className="text-green-600 font-medium">{patch.actualCounts?.n1.toLocaleString()}</TableCell>
                        <TableCell className="text-red-600 font-medium">{patch.actualCounts?.n2.toLocaleString()}</TableCell>
                        <TableCell className="text-blue-600 font-medium">{patch.actualCounts?.m1.toLocaleString()}</TableCell>
                        <TableCell className="text-orange-600 font-medium">{patch.actualCounts?.m2.toLocaleString()}</TableCell>
                        <TableCell className="text-purple-600 font-medium">{patch.actualCounts?.necrosis.toLocaleString()}</TableCell>
                        <TableCell className="text-pink-600 font-medium">{patch.actualCounts?.tumorCells.toLocaleString()}</TableCell>
                        <TableCell className="font-bold">
                          {patch.actualCounts ? Math.abs(patch.actualCounts.n1 - patch.actualCounts.n2) : "-"}
                        </TableCell>
                        <TableCell className="font-medium">{patch.actualCounts?.total.toLocaleString()}</TableCell>
                      </TableRow>
                    ))
                  ) : (
                    <TableRow>
                      <TableCell colSpan={10} className="text-center py-8 text-gray-500">
                        No suspicious patches found with the specified criteria.
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
